  ...
  def initVideo
    ...
    if $SUPPRESS_RENDER
      ...
      for frame_number in (0..@sti_playback.frames_count)
        ...
        #for col in (0..@video_playback.width-1)
        col = 300
          red = @viewport.getChromaticity(col, :red).histogram(:sturges, :bin_boundary => :avg, :min => 0, :max => 1)
          green = @viewport.getChromaticity(col, :green).histogram(:sturges, :bin_boundary => :avg, :min => 0, :max => 1)

          n_hist = (red[1].zip(green[1]).map {|x, y| (x > y) ? y : x })
          d_hist = [red[1], green[1]].min
          if (!(d_hist_prev.nil?))
            debugger
            debugger
          end
...
class Viewport
  ...
  def getChromaticity(col_num, type)
    ret = Array.new()
    colour_column = self.frame.unpack('C*')
    width_actual = col_num*3
    case type
      when :red
        for row in (0..height-1)
          pos_actual = (row*self.width*3)+width_actual
          red = colour_column[pos_actual]
          green = colour_column[pos_actual+1]
          blue = colour_column[pos_actual+2]
          chromaticity = (red.to_f / (red+green+blue))
          chromaticity = 1 if chromaticity.nan?
          ret << chromaticity
        end
      when :green
        for row in (0..height-1)
          ...
          green = colour_column[pos_actual+1]
          blue = colour_column[pos_actual+2]
          chromaticity = (green.to_f / (red+green+blue))
          chromaticity = 1 if chromaticity.nan?
          ret << chromaticity
        end
    end
